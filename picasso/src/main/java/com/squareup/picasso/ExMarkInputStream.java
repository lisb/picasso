...
public class ExMarkInputStream extends FilterInputStream {
    ...
    private int fillbuf(InputStream localIn, byte[] localBuf)
        ...
        if (markpos == -1 || (pos - markpos >= marklimit)) {
            if (localBuf.length > initialSize * 2) {
                // localBuf is too larger than initialSize.
                // shrink localBuf.
                markpos = -1;
                pos = 0;
                count = 0;
                ...
                localBuf = buf = new byte[initialSize];
            }
        ...
        if (markpos == 0 && marklimit > localBuf.length && pos >= localBuf.length) {
